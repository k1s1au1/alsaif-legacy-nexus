...
export function DesktopDashboardExtras(){
 ...
 useEffect(()=>{if(path!=="/dashboard"){setTarget(null);return}let cancelled=false;const find=()=>{if(cancelled)return;const el=document.querySelector(".max-w-6xl.mx-auto.space-y-12");if(el){setTarget(el);return}window.setTimeout(find,100)};find();return()=>{cancelled=true}},[path]);
 ...
 return createPortal(<div className="desktop-rebuild-root" dir="rtl">
   <section className="desktop-hero-card">
     ...
     <div className="desktop-hero-copy"><span>يا أهل الوفاء</span><h1>{name}</h1><p>كل خطوة تخطونها تبني مجدًا للعائلة.</p></div>
   </section>
