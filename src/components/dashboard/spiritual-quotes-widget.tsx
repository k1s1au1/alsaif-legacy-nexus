...
const SETTINGS_KEY = "faith_quotes";
...
function pickQuote(quotes: SpiritualQuote[]) {
  ...
  const pool = enabled.filter((q) => q.category === category);
  const fallbackPool = enabled.filter((q) => q.category === "general");
  const candidates = pool.length ? pool : fallbackPool.length ? fallbackPool : enabled;
...
export function SpiritualQuotesWidget({ variant = "dashboard" }: { variant?: "dashboard" | "desktop" }) {
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length) {
        const normalized = parsed.map((q: any) => ({
          ...
          type: ["quran", "hadith", "wisdom"].includes(q.type) ? q.type : "quran",
          category: ["general", "friday", "mon_thu", "white_days"].includes(q.category)
            ? q.category
            : "general",
      ...
      console.warn("Invalid faith quotes payload:", error);
    }
  }, []);
  ...
  const saveQuotes = async () => {
    ...
    if (error) {
      toast.error("تعذر حفظ النفحات", { description: error.message });
      return;
    }
  ...
  const manageButton = canManage ? (
    <button
      type="button"
      onClick={openManager}
      className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-[#b99755]/30 bg-white/70 px-3 py-1.5 text-[11px] font-black text-[#8b6b23] shadow-sm backdrop-blur transition hover:bg-white"
      title="إدارة نفحات إيمانية"
    >
      <Pencil size={13} />
      <span className="hidden sm:inline">إدارة النفحات</span>
    </button>
  ) : null;

  const content = variant === "desktop" ? (
    <div className="desktop-faith-strip relative">
      ...
      {manageButton && <div className="ms-auto">{manageButton}</div>}
    </div>
  ) : (
  ...
  return (
    <>
      {content}
      {showManager && typeof document !== "undefined" &&
        createPortal(
          <div className="fixed inset-0 z-[300] flex items-end justify-center bg-black/55 p-0 backdrop-blur-sm md:items-center md:p-6" dir="rtl">
            <div className="flex max-h-[92dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-[30px] bg-[#fffdf7] shadow-2xl md:rounded-[30px]">
              <div className="flex items-center justify-between border-b border-[#d8c79e]/35 px-5 py-4 md:px-7">
                <div>
                  <p className="text-xs font-black text-[#b08a39]">مسؤول القسم</p>
                  <h2 className="text-xl font-black text-[#0d4f3c]">إدارة نفحات إيمانية</h2>
                  <p className="mt-1 text-xs font-bold text-slate-500">
                    أضف أو عدّل الآيات والأحاديث والحِكم التي تظهر لأفراد العائلة.
                  </p>
                </div>
                <button type="button" onClick={() => setShowManager(false)} className="grid size-10 place-items-center rounded-full bg-black/5 text-slate-600 hover:bg-black/10">
                  <X size={19} />
                </button>
              </div>
              ...
              <div className="flex-1 space-y-4 overflow-y-auto p-5 md:p-7">
                {!loadedFromDatabase && (
                  <div className="rounded-2xl border border-[#b99755]/20 bg-[#b99755]/8 px-4 py-3 text-xs font-bold text-[#7b642d]">
                    هذه هي النفحات الافتراضية الحالية. أول حفظ سيجعلها قابلة للإدارة من القسم.
                  </div>
                )}

                {drafts.map((quote, index) => (
                  <article key={quote.id} className="rounded-3xl border border-[#d8c79e]/35 bg-white p-4 shadow-sm md:p-5">
                    <div className="mb-4 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <span className="grid size-8 place-items-center rounded-full bg-[#0d5b46] text-xs font-black text-white">{index + 1}</span>
                        <b className="text-sm text-[#0d4f3c]">نفحة إيمانية</b>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => updateQuote(quote.id, { enabled: !quote.enabled })}
                          className={`inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-[11px] font-black ${quote.enabled ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                        >
                          <Check size={12} /> {quote.enabled ? "مفعلة" : "مخفية"}
                        </button>
                        <button type="button" onClick={() => setDrafts((prev) => prev.filter((q) => q.id !== quote.id))} className="grid size-9 place-items-center rounded-full bg-rose-50 text-rose-600 hover:bg-rose-100">
                          <Trash2 size={15} />
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <label className="space-y-1.5 text-xs font-black text-slate-600">
                        النوع
                        <select value={quote.type} onChange={(e) => updateQuote(quote.id, { type: e.target.value as SpiritualQuote["type"] })} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-800">
                      ...
                      <label className="space-y-1.5 text-xs font-black text-slate-600">
                        وقت الظهور
                        <select value={quote.category} onChange={(e) => updateQuote(quote.id, { category: e.target.value as SpiritualQuote["category"] })} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-800">
                    ...
                    <label className="mt-3 block space-y-1.5 text-xs font-black text-slate-600">
                      النص
                      <textarea value={quote.text} onChange={(e) => updateQuote(quote.id, { text: e.target.value })} rows={3} placeholder="اكتب الآية أو الحديث أو الحكمة..." className="w-full resize-y rounded-2xl border border-slate-200 bg-white px-4 py-3 text-base font-bold leading-8 text-slate-900 outline-none focus:border-[#b99755]" style={{ fontFamily: "'Amiri', serif" }} />
                    </label>

                    <label className="mt-3 block space-y-1.5 text-xs font-black text-slate-600">
                      المصدر
                      <input value={quote.source} onChange={(e) => updateQuote(quote.id, { source: e.target.value })} placeholder="مثال: سورة آل عمران / رواه البخاري" className="h-11 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-800 outline-none focus:border-[#b99755]" />
                    </label>
                  </article>
                ))}

                <button type="button" onClick={addQuote} className="flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-[#b99755]/35 bg-[#b99755]/5 py-4 text-sm font-black text-[#8b6b23] hover:bg-[#b99755]/10">
                  <Plus size={18} /> إضافة نفحة جديدة
                </button>
              </div>

              <div className="flex items-center justify-end gap-3 border-t border-[#d8c79e]/35 bg-white px-5 py-4 md:px-7">
                <button type="button" onClick={() => setShowManager(false)} className="rounded-xl px-5 py-3 text-sm font-black text-slate-500 hover:bg-slate-100">إلغاء</button>
                <button type="button" onClick={saveQuotes} disabled={saving} className="inline-flex min-w-36 items-center justify-center gap-2 rounded-xl bg-[#0d5b46] px-6 py-3 text-sm font-black text-white shadow-lg disabled:opacity-60">
